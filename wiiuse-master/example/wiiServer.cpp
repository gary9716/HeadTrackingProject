...
#define MAX_WIIMOTES	((int)4)
#define NUM_IR_DOTS ((int)4)
...
int sendToPort = 13000;
char** irStrBuf;

int PadSize(int rawSize) {
  int pad = rawSize % 4;
  if (pad == 0)
    return rawSize;
  else
...
int InsertString(const char* s, char* packet, int start, int length) {
  int index = start;
  int sLen;
  int i = 0;
  for(;s[i] != 0;i++) {
...
int OscMessageToPacket(const char* addr, char * buffer,const int start,const int bufferSize) {
	int index = start;
	index = InsertString(addr, buffer, index, bufferSize);
	string tag;
	tag.push_back(',');
	int tagIndex = index;
	index += PadSize(2 + NUM_IR_DOTS);
	for(int i = 0;i < NUM_IR_DOTS;i++) {
		tag.push_back('s');
		index = InsertString(irStrBuf[i], buffer, index, bufferSize);
	}
	InsertString(tag.c_str(), buffer, tagIndex, bufferSize);
	return index;
}

void sendOscMsg(const char* addr) {
	int length = OscMessageToPacket(addr, sendBuffer, 0, sendBufferSize);
	sendto(sockfd,sendBuffer,length,0,(struct sockaddr *)&serverAddr,sizeof(serverAddr));
}

void sendIRInfoViaOsc(WCONST struct ir_t* ir) {
	//bool irDotsFound = false;
	//puts("ir data sent");
	for (int i = 0; i < 4; ++i) {
		/* check if the source is visible */
		int isVisible = 0;
		WCONST struct ir_dot_t* irDot = &(ir->dot[i]);
		if (irDot->visible) {
			isVisible = 1;
		}
		sprintf(irStrBuf[i],"ir:%i:%u,%u",isVisible,irDot->x,irDot->y);
	}
...
int main(int argc, char** argv) {
	...
	for(int i = 1;i < argc && i <= 2;i++) {
		if(i == 1) {
			...
			sendToPort = atoi(argv[i]);
		}
	}

	printf("dst IP:%s, port:%d\n",dstIP,sendToPort);

	irStrBuf = new char*[NUM_IR_DOTS];
	for(int i = 0;i < NUM_IR_DOTS;i++) {
		irStrBuf[i] = new char[250];
	}
